...
# 项目名称
SERVER_NAME=`sed -n '/spring.application.name/!d;s/.*=//p' ${CONF_DIR}/application.properties`

# 项目版本号
SERVER_VERSION=`sed -n '/version/!d;s/.*=//p' ${CONF_DIR}/version.txt`

#获取应用的端口号
SERVER_PORT=`sed -n '/server.port/!d;s/.*=//p' ${CONF_DIR}/application.properties`
...
#==========================================================================================
# JVM Configuration
# -Xmx2048m:设置JVM最大可用内存为256m,根据项目实际情况而定，建议最小和最大设置成一样。
# -Xms2048m:设置JVM初始内存。此值可以设置与-Xmx相同,以避免每次垃圾回收完成后JVM重新分配内存
# -Xmn1024m:设置年轻代大小为512m。整个JVM内存大小=年轻代大小 + 年老代大小 + 持久代大小。
#          持久代一般固定大小为64m,所以增大年轻代,将会减小年老代大小。此值对系统性能影响较大,Sun官方推荐配置为整个堆的3/8
# -XX:MetaspaceSize=512m:存储class的内存大小,该值越大触发Metaspace GC的时机就越晚
# -XX:MaxMetaspaceSize=512m:限制Metaspace增长的上限，防止因为某些情况导致Metaspace无限的使用本地内存，影响到其他程序
...
JAVA_OPTS=" ${JAVA_OPTS} -Xloggc:${LOG_DIR}/logs/${SERVER_NAME}_gc.log -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCTimeStamps -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=10 -XX:GCLogFileSize=100M"
...
# SERVER_PORT=`sed '/server.port/!d;s/.*=//p' config/application.properties`
